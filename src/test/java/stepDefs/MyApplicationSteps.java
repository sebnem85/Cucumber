...
public class MyApplicationSteps {
    ...
    public void user_gets_employee_info_for_employee_id_from_UI(String employee_id) throws Throwable {
        ...
        //  List<WebElement> elements = Driver.driver.findElements(By.xpath("//table//tr["+(i+1)+"]//td"));
    ...
    public void user_gets_data_for_employee_id_from_database(String employee_id) throws Throwable {
        String query="select e.first_name,e.last_name,j.job_title\n" +
                ...
                "e.job_id=j.job_id\n" +
                "where employee_id="+employee_id ;  // => we added that part to make our query dinamic +employee_id also we deleted employee_id=100 to make dinamic

        JDBCUtils.establishConnection();

     //   JDBCUtils.runSQLQuery(query);   // --> it will return me to list of maps
